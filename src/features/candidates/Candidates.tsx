    ...
    <Layout>
      ...
      {showModal && selectedCandidate && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-40 z-50 transition-opacity duration-700 ease-in-out">
          <div className="bg-white rounded-lg shadow-lg p-6 min-w-[350px] max-w-[90vw] w-full relative overflow-y-auto max-h-[90vh]">
            ...
            <button className="px-3 py-1 bg-primary text-white rounded mb-2" onClick={() => setShowTimelineModal(true)}>
              Show Timeline
            </button>
          </div>
        </div>
      ...
      {/* Timeline Modal */}
      {showTimelineModal && selectedCandidate && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-40 z-50 transition-opacity duration-700 ease-in-out">
          <div className="bg-white rounded-lg shadow-lg p-6 min-w-[350px] max-w-[90vw] w-full relative overflow-y-auto max-h-[90vh]">
            <button className="absolute top-2 right-2 text-gray-500 hover:text-gray-700" onClick={() => setShowTimelineModal(false)}>
              ×
            </button>
            <h3 className="text-xl font-bold mb-2">Candidate Timeline</h3>
